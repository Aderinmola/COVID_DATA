...
def load_data_to_db(file_name, table_name):
    ...
    covid_data.to_sql(table_name, con= engine, if_exists='replace', index= False)
    print('Data successfully written to PostgreSQL database!!!')

def main():
    download_google_drive_file("covid_19_data.csv")
    load_data_to_db("covid_19_data.csv", "covid_19_data")
